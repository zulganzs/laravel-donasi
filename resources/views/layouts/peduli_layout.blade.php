        ...
        <aside class="w-64 bg-white border-r border-gray-100 hidden md:flex flex-col z-10 shadow-sm">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <form method="POST" action="{{ route('logout') }}" class="px-2">
                    @csrf
                    <button type="submit" class="flex items-center w-full px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors duration-150 rounded-xl group">
            ...
            <header class="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 z-10 sticky top-0">
                ...
                <div class="flex items-center gap-4 ml-4">
                    <button class="p-2 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-full transition-all relative">
                     ...
                     <!-- Navbar Logout (Mobile/Desktop) -->
                    <form method="POST" action="{{ route('logout') }}" class="hidden md:block">
                        @csrf
                        <button type="submit" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-all" title="Keluar">
